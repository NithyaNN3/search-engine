...
# mongodb client and elasticsearch connected to search engine
app = Flask(__name__)
CORS(app)
es = Elasticsearch([{"host":"127.0.0.1","port":9200}],timeout=100) # elasticsearch server connection made here
myclient = MongoClient('mongodb://localhost:27017/')
database = myclient["scrapy"]
collection = database["products"] ### iterative feature needs to be added here

# basedir = os.path.abspath(os.path.dirname(__file__))
data_file = 'synonym.txt'
...
def create_index():
                ...
                "analysis": {
                    ...
                    "filter": {
                        "my_synonyms": {
                            "type": "synonym_graph",
                            "lenient": "true",
                            "synonyms_path": synonym_path,   ##add the path to synonyms.txt here
                            "updateable": "true"
                        },
                        "my_word_delimiter": {
                            "type": "word_delimiter",
    ...
    try:
        ...
        actions = []
        for data in tqdm(collection.find(), total=collection.count()):
            data.pop('_id')
            action = {
                "index": {
                    "_index": 'biomall',
        ...
        res = es.bulk(index = 'biomall', body = actions, refresh = True)
    ...
    try:
        res = es.search(index=indexname, body={"query": {
            ...
            "aggs": {                                          # Facets feature
                ...
                "quantity": {
                    "terms": {"field": "quantity", "size": 1000}
                }
        }, "size": size})
        res["by_brand"] = res["aggregations"]["by_brand"]["buckets"]
        res["country"] = res["aggregations"]["country"]["buckets"]
        res["packsize"] = res["aggregations"]["packsize"]["buckets"]
        return res
...
#        if field == "country":
#            return {"message": "Try 'country_name' instead of 'country'", "status_code": "400"}, 400
#        elif field == "brand":
#            return {"message": "Try 'brand_name' instead of 'brand'", "status_code": "400"}, 400
    ...
    if input == "*":
        ...
        res_1["country"] = res_1["search_response"]["aggregations"]["country"]["buckets"]
        res_1["quantity"] = res_1["search_response"]["aggregations"]["quantity"]["buckets"]
        return (res_1)
    pattern = re.compile('\W')
    ...
    try:
        ...
        res["country"] = res["search_response"]["aggregations"]["country"]["buckets"]
        res["quantity"] = res["search_response"]["aggregations"]["quantity"]["buckets"]
...
def update(indexname):
    ...
    bulk_data_syno = json.dumps(request.data)
...
app.run(host='0.0.0.0', port=5000, debug=True) # localhost server name and port